/*
 * Texxo Pro - Advanced Text Editor in C (GTK+ 3)
 * Compile with:
 *   gcc -o texxo texxo.c `pkg-config --cflags --libs gtk+-3.0` -lm
 * Requires GTK 3 development libraries and pkg-config.
 */
...
static const char* lookup_color(const char *name) {
    ...
    /* If it's a hex code starting with #, return as is */
...
static void update_line_numbers(TexxoEditor *app) {
    ...
    char *numbers = g_strdup("");
    for (int i = 1; i <= line_count; i++) {
        char buf[16];
        snprintf(buf, sizeof(buf), "%d\n", i);
        char *tmp = g_strconcat(numbers, buf, NULL);
        g_free(numbers);
        numbers = tmp;
    }
    GtkTextBuffer *buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(app->line_numbers_view));
    gtk_text_buffer_set_text(buf, numbers, -1);
    g_free(numbers);
}
...
static void update_status_bar(TexxoEditor *app) {
    ...
    char *text = gtk_text_buffer_get_text(app->text_buffer, 
                                          gtk_text_buffer_get_start_iter(app->text_buffer),
                                          gtk_text_buffer_get_end_iter(app->text_buffer), FALSE);
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "Lines: %d | Words: %d | Characters: %d | Position: %d:%d",
             lines, words, chars, line, col);
...
static void apply_font_style(TexxoEditor *app, const char *style) {
    /* style corresponds to a key in font_map */
...
static void undo(TexxoEditor *app) {
    gtk_text_buffer_undo(app->text_buffer);
    update_status(app, "Undo", FALSE);
}

static void redo(TexxoEditor *app) {
    gtk_text_buffer_redo(app->text_buffer);
    update_status(app, "Redo", FALSE);
}
...
static void insert_image(TexxoEditor *app, const char *path) {
    ...
    /* Scale large images to fit width */
    ...
    if (width > max_width) {
        ...
        GdkPixbuf *scaled = gdk_pixbuf_scale_simple(pixbuf, max_width, new_height,
                                                     GDK_INTERP_BILINEAR);
...
static void execute_command(TexxoEditor *app, const char *cmd) {
    /* Add to history */
    app->command_history = g_list_prepend(app->command_history, g_strdup(cmd));
    if (g_list_length(app->command_history) > 50) {
        GList *last = g_list_last(app->command_history);
        g_free(last->data);
        app->command_history = g_list_delete_link(app->command_history, last);
    }
    app->history_index = -1;
    ...
    char *args = cmd_copy + strlen(token) + 1; /* rest of string */
    while (*args == ' ') args++;
    ...
    if (strcasecmp(token, "bo") == 0 || strcasecmp(token, "bold") == 0)
        ...
        undo(app);
    else if (strcasecmp(token, "redo") == 0)
        redo(app);
        ...
        if (old_text && new_text) {
            ...
            if (gtk_text_buffer_get_selection_bounds(app->text_buffer, &start, &end)) {
                ...
                char *rep = g_strjoinv(new_text, g_strsplit(sel, old_text, -1)); /* simplified */
                gtk_text_buffer_delete(app->text_buffer, &start, &end);
        ...
        GtkTextIter start, end;
        gtk_text_buffer_get_bounds(app->text_buffer, &start, &end);
        gtk_text_buffer_select_range(app->text_buffer, &start, &end);
    }
        ...
        /* theme toggle */
        app->theme = (strcasecmp(args, "light") == 0) ? 1 : 0;
        /* In a full implementation, change widget styles */
    }
    else {
...
static void activate(GtkApplication *gtk_app, gpointer user_data) {
    ...
    g_signal_connect_swapped(new_item, "activate", G_CALLBACK(new_document), app);
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), new_item);
    
    GtkWidget *open_item = gtk_menu_item_new_with_label("Open");
    g_signal_connect_swapped(open_item, "activate", G_CALLBACK(open_file), app);
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), open_item);
    
    GtkWidget *save_item = gtk_menu_item_new_with_label("Save");
    g_signal_connect_swapped(save_item, "activate", G_CALLBACK(save_file), app);
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), save_item);
    
    /* Edit, Format, Image menus similar ... (abbreviated for brevity) */
    
    gtk_box_pack_start(GTK_BOX(app->main_vbox), menubar, FALSE, FALSE, 0);
    
    /* Toolbar */
    app->toolbar = gtk_toolbar_new();
    gtk_toolbar_set_style(GTK_TOOLBAR(app->toolbar), GTK_TOOLBAR_ICONS);
    /* Add buttons (using stock items) */
    GtkToolItem *new_btn = gtk_tool_button_new_from_stock(GTK_STOCK_NEW);
    g_signal_connect_swapped(new_btn, "clicked", G_CALLBACK(new_document), app);
    gtk_toolbar_insert(GTK_TOOLBAR(app->toolbar), new_btn, -1);
    /* ... more buttons */
    gtk_box_pack_start(GTK_BOX(app->main_vbox), app->toolbar, FALSE, FALSE, 0);
    ...
    gtk_widget_set_margin_top(cmd_box, 5);
    ...
    gtk_css_provider_load_from_data(theme_provider,
        ...
        "toolbar { background-color: #0a1a3a; }\n"
...
int main(int argc, char **argv) {
    GtkApplication *app = gtk_application_new("com.texxo.pro", G_APPLICATION_FLAGS_NONE);
